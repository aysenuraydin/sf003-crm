<script setup>
    ...
    onMounted(() => {
        axios.get('/Regions').then(result => {
            if (result && result.data) allRegions.value = result.data;
        })
    })
    ...
    function saveItem() {
        ...
        if (item.id > 0) {
            axios.put("/Territories/" + item.id, item).then(res => {
                toastr.success("Territory updated!", "Updated")
                emit("onSaved", item)
            })
        } else {
            axios.post("/Territories", item).then(res => {
                toastr.success("Territory created!", "Created")
                emit("onSaved", item)
            })
        }
    }
                ...
                <form @submit.prevent="true">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-9">
                                <select v-model="item.regionId" class="form-select">
